    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            v2f vert(appdata v)
            {
                ...
                float3 positionWS = TransformObjectToWorld(v.positionOS); // в книге матрицейЮ посмотрим
                ...
                float3 halfVector =
                normalize(mainLight.direction + viewWS);
                float specular = max(0, dot(normalWS,
                                            halfVector));
                specular = pow(specular, _GlossPower);
                float3 specularColor = mainLight.color *
                specular;
                o.diffuseLighting = float4(ambient + diffuse,
                                           1.0f);
                o.specularLighting = float4(specularColor,
                                            1.0f);
                return o;
            }
